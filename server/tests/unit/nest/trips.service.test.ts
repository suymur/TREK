...
describe('folded trip CRUD', () => {
  ...
  it('TRIP-SVC-063: copying a trip copies the installments of an expense, all open again', () => {
    ...
    const rows = testDb.prepare(`
      SELECT i.label, i.amount, i.due_date, i.paid_at, i.sort_order FROM budget_item_installments i
      JOIN budget_items b ON b.id = i.budget_item_id WHERE b.trip_id = ? ORDER BY i.sort_order
    `).all(newTripId);
    expect(rows).toEqual([
